exports.run = (client, message, args, level) => {
	if (!args[0]) {
		...
		sorted.forEach(c => {
			...
			output += `• ${message.settings.prefix}${c.help.name}${' '.repeat(longest - c.help.name.length)} :: ${c.help
				.description}\n`;
		});
...
exports.help = {
	...
	extended:
		'This command will display all available commands for your permission level, with the additonal option of getting per command information when you run \'commands [command name]\'.',
