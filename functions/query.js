...
exports.insertGuild = async (client, guild) => {
  await sql.run(
    'INSERT INTO guild_config (gID, prefix, feedbackChannel, botLogEnable, modRole, adminRole, enableBadges, deleteSwitch, response, pinMessage, messageID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [guild.id,
...
exports.updateUser = async (client, message, type) => {
  if (type === 'submit') {
    ...
    await sql.run(
      'UPDATE users SET timesRequested=?, tokens=?, lastRequest=?, keywordCount=? WHERE jID=?',
    ...
    await sql.run(
      'UPDATE guild_config SET messageID=? WHERE gID=?',
      [message.id,
        message.guild.id,
      ],
    );
...
exports.setConf = async (client, message, settings) => {
  await sql.run(
    'UPDATE guild_config SET prefix=?, feedbackChannel=?, botLogEnable=?, modRole=?, adminRole=?, enableBadges=?, deleteSwitch=?, response=?, pinMessage=?, messageID=? WHERE gID=?',
    [settings.prefix,
...
exports.loadConfig = async (client, message) => {
  await sql.get(`SELECT * FROM guild_config WHERE gID = "${message.guild.id}"`).then((row) => {
    if (row) { message.settings = row; } else { message.settings = client.config.defaultSettings; }
  }).catch(() => { console.error; });
};
...
exports.userToken = async (client, target, row) => {
  await sql.run(
    'UPDATE users SET tokens=? WHERE jID=?',
    [row.tokens,
      target.joined,
    ],
  );
